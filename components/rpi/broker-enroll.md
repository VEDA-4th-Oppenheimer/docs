...
**1회용 토큰 하나와 맞바꿔** 내려주는 HTTPS 서비스다. RPi 에 상주하며 브로커
ACL 까지 같이 갱신한다.
...
프로세스라 수명주기가 다르다. 같은 빌드에 묶으면 데몬을 고칠 때마다 발급 서비스가
재기동된다.

## 2. 왜 배포본에 인증서를 넣지 않는가

Qt 배포본에는 인증서도 카메라 설정도 담지 않는다.

- 인증서에는 **`adts/cmd/#` 쓰기 권한**이 있다 — 받은 사람이 장비를 움직일 수 있다.
- 카메라 설정에는 **카메라 admin 비밀번호가 RTSP URL 에 박혀** 있다.

배포물에 넣으면 받은 사람 **전원**이 그 권한을 갖는다. 그래서 사람마다 다른 CN 으로
따로 발급하고, 그 통로가 이 서비스다.

반대 방향의 신뢰는 Qt 실행파일에 박아둔 `ca.crt` 가 담당한다. 발급 시점에는 클라이언트
인증서가 아직 없어서 서버를 검증할 근거가 그것뿐이다. `ca.crt` 는 **공개** 인증서라
배포본에 들어가도 안전하고, CA 를 재발급하면 이 파일도 같이 갱신해야 한다.
...
**토큰 소비가 마지막인 것이 핵심이다.** 3절의 순서를 지키지 않으면 사용자가 아무것도
못 받았는데 토큰만 사라진다(5.2절).
...
> **401 은 "틀렸다"와 "이미 썼다"를 구분해 주지 않는다.** 구분해 주면 공격자가 유효한
> 토큰 공간을 좁힐 수 있다. 로그에는 둘 다 남는다.

**`mqtt.host` 의 기본값은 요청의 `Host` 헤더**다. 클라이언트가 IP 로 접속하면 그 IP 가
그대로 내려가므로 대개 설정할 필요가 없고, 브로커가 다른 호스트에 있을 때만
`ADTS_MQTT_HOST` 로 지정한다.

**`cameras` 는 없어도 진행한다.** 없으면 경고만 남기고 응답에서 뺀다 — MQTT 는 되고
영상만 안 나오는 상태가 된다. 등록 자체를 실패시키면 카메라 설정이 준비되기 전에는
아무도 못 붙는다.
...
파일은 `/etc/adts/enroll_tokens`, 권한 600. 한 줄에 하나다.
...
토큰 생성 시점에 정하게 하려는 것이다.

토큰은 `openssl rand -hex 24`(24바이트 = 48자)로 만든다.

### 5.2 왜 검증과 소비를 나눴나

`token_take(token, label, commit)` 이 `commit` 플래그로 두 번 불린다.
...
> 예전에는 **찾자마자 지우고** 인증서를 발급했다. 발급이나 ACL 갱신이 실패하면
> 사용자는 아무것도 못 받았는데 토큰은 사라져서, 관리자가 새 토큰을 만들어 주기 전까지
> 재시도할 방법이 없었다.

교체는 `<파일>.tmp` 에 쓴 뒤 `chmod 600` → `rename()` 이다. `rename` 이 원자적이라
중간에 죽어도 반쪽짜리 토큰 파일이 남지 않는다.

**소비 단계에서 실패하면 응답은 그대로 보낸다.** 인증서는 이미 나갔으므로 사용자를
막을 이유가 없고, 대신 "같은 토큰이 다시 쓰일 수 있음" 경고를 로그에 남겨 관리자가 손으로
지우게 한다.

> 검사-후-소비 사이에 다른 요청이 끼어들 수 없는 것은 **단일 스레드 accept 루프**이기
> 때문이다(9절). 나중에 동시 처리를 넣으면 이 가정이 깨진다.
...
`strcmp` 는 첫 불일치에서 끊긴다. 그 시간 차로 토큰을 앞에서부터 한 글자씩 맞춰 갈 수
있어서 OpenSSL 의 상수 시간 비교를 쓴다. 길이는 미리 비교한다.
...
- **`--list-tokens` 는 토큰 전문을 찍지 않는다** — 앞 6자와 뒤 4자만 보여준다. 화면이나
  터미널 로그에 남으면 그 자체가 접근 권한이다.
- **라벨 검증은 토큰을 만들 때** 한다(영숫자 + `.` `_` `-`). 발급 시점에 걸러면 "토큰은
  받았는데 발급이 500" 이 된다. CN 은 파일명과 ACL 한 줄에 그대로 들어가므로 문자 집합을
  좁혀야 경로 조작·ACL 구문 훼손을 막는다. 서비스 쪽에도 `cn_is_safe()` 로 같은 검사가
  한 번 더 있다.
- 같은 라벨의 미사용 토큰이 이미 있으면 경고한다. 여러 개 있어도 동작은 하지만(먼저 쓴
  것만 유효) 관리자가 헷갈린다.

## 6. 인증서 서명

서명 로직을 서비스 안에 다시 구현하지 않고 **`gen-certs.sh --client <CN>` 을 `fork`/`exec`
로 호출**한다. 여기서 다시 짜면 확장(`v3_client`)이나 키 포맷 같은 세부가 두 곳으로
갈라진다.
...
**셸을 거치지 않는다**(`execv`) — 인자에 무엇이 들어와도 명령 인젝션이 성립하지 않는다.
`bash` 를 명시적으로 부르는 이유는 스크립트의 실행권한과 shebang 에 의존하지 않기 위해서다.

서명 결과에서 세 가지가 중요하다.

| 항목 | 이유 |
|---|---|
| `-extensions v3_client` | 빠지면 **mTLS 핸드셰이크에서 거부**된다 |
| 전통 RSA 포맷 `<CN>-trad.key` | Qt `QSslKey` 가 OpenSSL 3.x 기본인 PKCS#8 을 `QSsl::Rsa` 로 읽으면 **null 을 반환하고 조용히 실패**한다. 그래서 변환본을 같이 만들어 이걸 내려준다 |
| CA 키는 장비 밖으로 안 나감 | 서명은 전부 이 프로세스 안에서 이뤄진다. 그래서 서비스가 root 로 돈다 |

**재발급**(로그아웃 후 재등록 등)이면 기존 파일을 지우고 새로 만든다.

> 주의: **파일을 지운다고 이전 인증서가 폐기되지 않는다.** 암호학적으로 여전히
> 유효하다. 실제로 무효화하려면 브로커에 `crlfile` 을 걸고 폐기 목록에 올려야 한다.
> 지금은 CRL 이 없다(11절).

## 7. ACL 자동 등록 — 가장 놓치기 쉬운 곳

mosquitto 의 `user` 는 **정확 매칭이고 와일드카드가 없다.** 발급할 때마다 CN 블록을
직접 붙여야 한다.
...
> **빠뜨리면 TLS 핸드셰이크는 성공하는데 구독·발행만 조용히 막힌다.** 인증서도 정상,
> 연결도 정상, 그런데 아무 메시지도 안 오간다. 원인을 찾기 가장 어려운 형태라
> 발급 서비스가 자동으로 붙이도록 만들었다.
...
Client ID 를 쓰는데, 그래도 ACL 은 그대로 쓸 수 있는 이유가 이것이다.

## 8. 스캔 파일 조회

Qt 가 `state/scan` 으로 파일 **경로**를 받아도 그 파일을 실제로 가져갈 수단이 없었다.
발급 서비스에 읽기 전용 경로를 얹어 **이미 있는 mTLS 신뢰를 재사용**한다.
...
인증서를 발급하는 서비스에 파일 경로를 여는 것이라 **세 겹으로 막는다.**
...
> 인코딩이 필요 없다. 디코더를 두면 그 자체가 새로운 우회 표면이 된다.

`/scans` 는 **경로를 주지 않는다.** 파일명·크기·수정시각만 준다 — 클라이언트는 파일명만
알면 되고 서버가 디렉터리를 고정하고 있어서 경로를 노출할 이유가 없다. 한 번에 최대
500건이다.
...
두 경로의 요구가 정반대라서 이렇게 된다.

- `/enroll` 은 **인증서를 발급받기 전에** 부르는 곳이라 인증서를 요구할 수 없다.
- `/scan`·`/scans` 는 파일을 내주므로 검증된 인증서를 요구한다.

그래서 핸드셰이크는 누구나 되게 두고, **판정을 핸들러에서** 한다. 최소 버전은 TLS 1.2.

클라이언트 CA 를 못 읽으면 경고만 남기고 계속 뜬다 — `/enroll` 은 그래도 돌아야 하기
때문이다. 대신 `/scan` 계열은 그 상태에서 항상 거부된다.

### 9.2 단일 스레드 + 소켓 마감시간

한 연결씩 처리한다. 발급은 사람이 하루 몇 번 하는 작업이라 성능이 문제되지 않고, 토큰
파일과 ACL 파일을 동시에 건드릴 일이 없어 **잠금 없이 안전**한다. 인증서를 다루는
코드에서는 그 단순함이 장점이다.

대신 한 연결이 멈추면 **그동안 아무도 발급받지 못하고 스캔 다운로드도 막힌다.**

```c
#define CLIENT_TIMEOUT_S 20        /* SO_RCVTIMEO / SO_SNDTIMEO */
```

> 이게 없으면 **접속만 하고 아무것도 안 보내는 클라이언트 하나로 서비스 전체가 정지**한다
> (`SSL_accept` 의 첫 read 에서 무한정 기다림). 단일 스레드라 이 값이 그대로 "서비스 전체가
> 멈출 수 있는 시간" 이다.

`SIGPIPE` 는 무시한다 — 클라이언트가 먼저 끊어도 프로세스가 죽지 않게.
...
> 실행 중인 바이너리를 덮어쓰면 `Text file busy` 가 난다. **stop → cp → start** 순서로
> 할 것.
...
끝**이라, 카메라 IP 가 바뀌면 이미 등록한 사람에게 전달할 방법이 없었다. 등록 시점에
서버에 `cameras.json` 이 없었던 사람은 영구히 영상 없이 남기도 했다(실제로 겪음).
그래서 `publish-config.sh` 로 `adts/config/cameras` 에 **retained** 발행하도록 만들었다.
나중에 켜는 콘솔도 접속 즉시 현재 값을 받아야 해서 retained 였다.

**뺀 이유** (`af1adf9`, 2026-08-05): 카메라는 RPi 와 **물리적으로 떨어져 있고 데몬은 카메라를
건드리지 않는다.** RTSP 는 Qt 가 카메라로 직접 연결하므로 RPi 를 배포처로 삼을 이유가
없었다. 오히려 서버에 `cameras.json` 이 없으면 "등록은 성공했는데 영상만 없음" 이 되어
원인을 찾기 어려웠다(두 번 발생). Qt 가 등록 화면에서 카메라 정보를 직접 받도록
바뀌면서 이쪽을 지웠다 — `publish-config.sh` 삭제, ACL 에서 `adts/config/#` 권한 제거,
발급 서비스가 새 CN 블록에 붙이던 config 읽기 줄 제거.

> 남은 절충: 지금도 서버에 `cameras.json` 이 있으면 `/enroll` 응답에 실어 보낸다. 다만
> **폴백**이고, 사용자가 등록 화면에서 넣은 값이 우선이다.

### 11.2 `GET /scans` 가 배포 직후 404

두 값이 저장소 안에서 어긋나 있었다(`9ae2f9f`, 2026-08-07).

- `DEF_SCAN_DIR` 이 `/home/pi/final_project/scans` 인데 유닛에 **`ProtectHome=true`** 가
  걸려 있었다. 홈 아래는 경로가 맞아도 열리지 않으므로 그 기본값은 성립할 수 없다.
- 데몬(`SCAN_OUT_DIR`)은 `/var/lib/adts/scans` 를 1순위로 쓰지만, 그 디렉터리가 없으면
  **작업 디렉터리의 `./scans` 로 조용히 폴백**한다. 데몬을 홈에서 띄우면 파일은 홈 아래
  쌓이고 서비스는 하나도 못 보는 상태가 된다.

기본값을 데몬과 같은 `/var/lib/adts/scans` 로 맞추고 유닛에도 명시했다. **값을 바꾼 게
아니라 원래 의도였던 곳으로 되돌린 것**이다.

### 11.3 `pipefail` 에 토큰만 날아감

`--new-token` 이 전달문에 넣을 서버 IP 를 `hostname -I` 로 추정하는데, 이건 리눅스
전용이라 없는 환경이 있다. `set -o pipefail` 이 켜져 있어서 그 실패가 그대로 올라오면
**토큰은 이미 파일에 쓰인 뒤라 안내문 없이 죽는다.** 실제로 겪었고,
`{ hostname -I 2>/dev/null || true; }` 로 감싸 해결했다.

## 12. 알려진 한계

**① CRL 이 없다.** ACL 블록을 지우면 그 CN 으로 아무것도 못 하게 되지만, **인증서 자체는
여전히 유효해서 브로커 연결(TLS 핸드셰이크)은 된다.** 기기 분실 대응이 필요하면
`mosquitto.conf` 에 `crlfile` 을 걸고 인증서를 폐기 목록에 올려야 한다.

**② 실장비 왕복 검증이 안 된 구간이 있다.** `GET /scans`·`GET /scan/<파일명>` 은
실장비에서 재빌드·재기동해 끝까지 왕복 확인한 적이 없다(`84251db` 커밋 메모).

**③ 동시 처리를 넣으면 토큰 로직이 깨진다.** 5.2절의 검사-후-소비가 단일 스레드 가정 위에
서 있다. 잠금 없이 스레드를 붙이면 같은 토큰으로 두 장이 발급될 수 있다.

**④ 공용 CN `qt-console` 이 아직 ACL 에 남아 있다.** `gen-certs.sh` 전체 발급이 만드는
개발용 인증서다. 사람별 발급으로 완전히 전환하면 지워야 한다.

**⑤ 토큰 만료가 없다.** 1회용이지만 시간 제한은 없어서, 나눠주고 안 쓴 토큰은 회수하기
전까지 계속 유효하다.

...
UART 통신 드라이버 구현 내용을 정리한 것이다. STM32 MCU가 라이다로부터 거리 데이터를
수신·파싱·검증하여 최종적으로 각도(θ, φ) 데이터와 결합해 Raspberry Pi 4B로 전달하기까지의
파이프라인 중 **앞단(센서 파싱 + 각도 결합)** 에 해당한다.

> **각도 결합까지가 이 모듈의 책임이다.** 초기 설계에서는 `lidar_get_distance_mm()` 을
> 상위 로직이 폴링해 각도와 짝짓는 구조였으나, 그 방식은 스캔 정밀도를 만족하지 못해
> **프레임 완성 시점에 ISR 에서 각도를 래치하는 구조로 변경**되었다(4.3절). 폴링 API 는
> 디버그용으로만 남아 있다.

**센서 사양**: TOF(Time of Flight) 방식, 거리 분해능 1mm, 최대 갱신 주파수 100Hz,
FOV 1~2°. 측정 거리는 데이터시트 재확인이 필요하다(9절 ①).
...
8N1 / 115200bps(가변)이다. 한 프레임은 고정 16바이트로 구성된다.
...
**엔디안**: NLink는 Little-Endian을 따른다. 거리값은
`buf[8] | (buf[9]<<8) | (buf[10]<<16)` 로 추출한다.
...
**실측 참고**: 1축 브링업에서 유효점 359/359 가 전부 `1` 로 수신되었다.

> **주의 — 파서는 Dis Status로 거르지 않는다.** 초기 구현은 "0과 1만 통과"시켰으나
> 현재는 **체크섬만 검사하고 원본 값을 그대로 상행**한다. 변경 이유는 4.5절 참조.
...
| 출력 레이트 | F2 P 액티브 출력 기본 50Hz — **100Hz로 설정해 사용**. 스캔 샘플 간격이 `틸트속도 / rate` 로 결정되므로 50Hz면 격자가 절반으로 성기다 |

`main.c` 에서의 결선은 네 곳이다. `RxCplt`/`Error` 콜백은 USART1(RPi)과 공유되므로 각
모듈이 `huart->Instance` 를 확인한 뒤 자기 것만 처리한다.
...
UART/인터럽트 제어와 순수 패킷 파싱을 분리해 재사용성과 테스트 용이성을 확보했다.
...
> **`calib.c / .h` 는 삭제되었다**(커밋 `5b229cb`). 오프셋 보정·EMA 필터는 펌웨어에서
> 수행하지 않는다 — 원본 raw 데이터를 그대로 RPi로 올리고 보정은 상위(데몬/캘리브)에서
> 한다. 근거는 4.5절과 같다.
...
경계는 하나다. **ISR이 생산하고 메인루프가 소비한다.** ISR에서 하는 일은 바이트
...
거리와 각도를 짝짓는 **시점**이 이 모듈 설계의 핵심이다.

폴링(메인루프에서 `lidar_get_distance_mm()` 을 읽어 각도와 결합)으로는 요구 정밀도를
만족할 수 없다. 메인루프가 언제 읽느냐에 따라 최대 한 프레임 주기(100Hz = 10ms)만큼
어긋나는데, 틸트가 90°/s 로 도는 동안 그 사이에 **0.9°** 를 움직이다. 스캔 격자가
0.9°인데 스미어가 0.9°면 격자 한 칸이 통째로 뭉개진다.

따라서 프레임의 **마지막 바이트를 받은 그 순간** `scan_latch_angles()` 로 모터 카운터를
읽어 샘플에 함께 담는다. 카운터 읽기뿐이라 ISR에서 안전하다.

래치 시각은 `stm_ts_ms`(STM32 HAL tick)로 함께 상행한다. 라이다 자체 시계인
`device_time_ms` 와는 **서로 다른 clock domain 이므로 섞어 쓰면 안 된다.**
...
- 생산자 = RX ISR(`g_q_head` 만 증가), 소비자 = 메인루프(`g_q_tail` 만 증가). 각각 한쪽에서만
  증가하는 8비트 단일 워드 접근이라 **락이 필요 없다.**
- 깊이가 2의 거듭제곱이라 인덱스는 마스킹(`& (LEN-1)`)으로 감싼다.
- 8칸 = 100Hz 기준 **80ms** 여유. 실제로는 한 칸을 넘지 않는다 — 메인루프에서 가장 느린
  구간이 점 하나 상행(115200에서 23바이트 ≈ 2ms)이기 때문이다.

**넘칠 때는 새 샘플을 버린다.** 가장 오래된 것을 밀어내지 않고 새 것을 버린 뒤
`queue_drops` 를 올린다 — 이미 들어간 샘플은 각도가 짝지어져 있고, 순서를 흐트러뜨리면
데몬이 격자에 넣을 때 더 헷갈리기 때문이다. `drops` 증가는 라이다가 아니라 **메인루프가
80ms 이상 밀렸다**는 신호다.
...
그 점은 영영 복구되지 않는다. 실제로 1축 브링업에서 거리 상한 필터 때문에 먼 벽이
잘려나간 적이 있고, 그때 쓰던 상한 값 자체도 사양과 달랐다. `calib` 모듈을 걷어낸
것도 같은 이유다.

유일한 예외는 거리 폭 변환이다. 프레임은 u24, 프로토콜은 u16(mm)이라 `lidar_process()`
에서 `0xFFFF` 로 클램프한다. 사양 상한 내에서는 항상 그대로 들어가며, 범위를 넘는 값은
센서 오류이지만 **버리지 않고** Dis Status와 함께 올려 데몬이 판정하게 한다.
...
RPi에 올라간다. `scan` 은 **스윕 구간(`SC_SWEEP`)의 점만** 상행한다 — 줄 끝 정지나
팬 이동 중의 점은 격자에 넣을 각도가 아니라 같은 각도에 중복만 만들기 때문이다.
...
`HAL_UART_Receive_IT()` 로 재무장한다.

> **주의**: 워치독(IWDG)은 이 정지와 무관하게 계속 정상 급여(refresh)되므로, **라이다만
> 조용히 멈춰 있는 상황이 겉으로 드러나지 않는다.** 7절의 진단 카운터가 그 감지 수단이다.

---

## 7. 진단 — "라이다가 안 온다"를 원인별로 가르기

1축 브링업에서 원인을 좁히는 데 하루가 걸렸다. 아래 조합을 보면 1분 안에 갈린다.
...
`rx_bytes` 카운터가 없으면 1행과 2행이 구분되지 않아 추가했다.

수신이 아예 기동하지 않는 경우는 `lidar_get_uart_diag()` 로 본다.
...
오류율이 나온다. 마지막 유효 프레임의 원본 필드는 `lidar_get_last_frame()` 으로 확인한다.

---

## 8. 정적분석 관련 메모

`App/` 은 MISRA-C:2012 게이트 대상이다. 이 모듈의 개별 deviation 은 다음 두 가지이며,
모두 근거 주석과 함께 룰 단위로 억제한다.
...
폴더 통째 억제로 되돌리지 않는다 — 정적분석 가이드 §2 참조.

---

## 9. 알려진 이슈 및 미해결 항목

**① Dis Status = 1 상시 관측** — 현 보드/환경에서 대부분 `1` 로 수신된다. 2.2절 표에
따르면 경고 수준(표준편차 15mm 초과)이므로 이 값만으로 품질을 판단하지 말고, 실측 오차가
예상보다 크면 설치 각도 · 표적 반사율 · 측정 거리를 재점검할 것.
---

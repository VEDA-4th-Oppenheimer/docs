...
| 대상 소스 | `App/scan/scan.c` (809줄), `scan.h` (273줄) |
...
| 다루는 것 | 다루지 않는 것 |
|---|---|
| 상태머신과 전이 조건 | 모터 구동·가감속 램프 (강유근) |
| 홈 확립 절차와 판정 | MT6701 판독·I2C 복구 (강유근) |
| 목표 각도 계산 | 라이다 NLink 파싱 (송영빈) |
| 상행 경계 (`scan_submit_sample`) | 프레임 조립 → 60 uart_rpi 프로토콜 어댑터 |
...
> **STM-61-01 (P0) — 임계 2.0° 의 여유가 얇다**
> 헤더 주석 자체가 데드밴드를 1.8°, 엔코더 지터를 0.42° 로 잡고 있는데
> 임계가 2.0° 라 여유가 0.2° 뿐이다.
>
> 판정은 줄 끝 200회 + 팬 스텝 199회 = **399회** 일어난다(마지막 줄에는 팬 스텝이
> 없다). 한 번만 걸려도 `motor_disarm` + `SC_IDLE` 로 스캔이 중단된다.
>
> 중단돼도 산출물이 아예 없는 것은 아니다 — 데몬은 `CMD_SCAN_DONE` 을 못 받아도
> 무입력이 `SCAN_IDLE_TIMEOUT_MS` 를 넘으면 `ST_EXPORT` 로 넘어가 **그때까지의
> 점으로 파일을 마감한다**(`RPi/daemon/core/main.c:713~740`). 잃는 것은 남은
> 줄이고, 남는 것은 부분 격자다.
>
> 동일 구조가 과거 줄 끝 재영점 방식에서 헛 `ERR_STALL` 을 발생시켰다. 중앙값 필터와
> 정착 대기가 완화하지만, 데드밴드에 걸린 정지 위치가 한쪽으로 치우친 상태에서
> 지터가 겹치면 임계를 넘길 수 있다.
>
> 기준선에서 정착이 40ms 다(§3.3). 완화 요인 중 대기 쪽이 100ms 시절의 40% 로
> 줄었으므로 S-Curve 의 링잉 감소분이 그만큼을 메워야 한다. 40ms 실기 2회가
> 399회 판정을 통과했다는 것이 현재 가진 유일한 관측이고, 오차 분포 자체를
> 잰 기록은 없다.
>
> 반증 가능한 검증: RPi `driver/encoder_jitter_test` 로 무전류·정지 상태의 지터
> 분포를 양축에서 측정하고, 표준 스캔의 줄 끝 오차 실측 분포와 2.0° 사이 여유를
> 수치로 확인한다. 부족하면 임계를 올리거나 N회 연속 위반을 조건으로 바꾼다.

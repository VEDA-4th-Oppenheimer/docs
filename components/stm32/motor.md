...
```text
================================================================================
          [ 틸트 메인 암 2D 스케치 치수 및 3D 프린팅(Infill 40%) 동역학 구조도 ]
================================================================================

  [ 1. 2D 정밀 스케치 및 80.00mm 모멘트 암 치수 ]
       |<── 40.00 mm ───>|<─── 40.00 mm ───>|
  5.00 |                 |                  |
  <──> |                 |                  |
   ┌───┬─────────────────┬──────────────────┐ ▲
  /    │                 │                  │ │ 15.00 mm (상단 라이다 지지 탭)
 │ (○) │ (중심선)        │                  └──┘ ▼
 │Ø5.2 │─────────┬───────┴───────────── 15.00 mm (중심선)
  \    │         │ 4.50mm / 2.60mm             ▲
   └───┴─────────┴─────────────────────────────┘ │ 30.00 mm (전체 폭)
       │                                       ▼
       |<────────────── 80.00 mm ──────────────>| (돌출 깊이: 30.00 mm)

  [ 2. FDM 3D 프린팅 Infill 40% 내부 단면 구조 (38.8g) ]
  ...
  [ 3. 스윕 종료 후 진동 소멸 타임라인 및 40ms 정착 대기 마진 ]
  진동 진폭(A)
   ▲  |\
   │  | \      [진동 감쇠 곡선]
   │  |  \    /\
   ...
   │  |    \/    \/  \─────────── (25.4ms: 진동 98% 완전 소멸)
   └──+─────+─────+────+───────────+─────────────────────────► 시간(t)
     0ms   8ms   16ms 25.4ms     40.0ms (SCAN_LINE_SETTLE_MS 만료)
      |←─── 진동 소멸 ───→|←+14.6ms 마진→| (하드웨어 안전 검증 완료)
================================================================================
```

* **틸트 기구물 2D 스케치 역해석 및 행간 정착 시간(40ms) 동역학 검증 근거**:
  * **2D 스케치 치수 및 체적 정합성**: 회전축 중심($\varnothing 5.20\,\text{mm}$)에서 라이다 끝단까지의 모멘트 암 길이는 **$80.00\,\text{mm}$**이며, $30 \times 30\,\text{mm}$ 단면 돌출 체적($56,100\,\text{mm}^3$)은 솔리드웍스 모델링 체적($56,100.48\,\text{mm}^3$)과 $100\%$ 일치한다.
  * **3D 프린팅(Infill 40%) 실효 무게 도출**: 외벽 3-Wall($1.2\,\text{mm}$) 및 내부 Infill 40% 격자 구조 적용 시, 실제 출력물 무게는 **`약 38.8g`** ($100\%$ 솔리드 $79.66\,\text{g}$ 대비 $51\%$ 경량화)으로 산출된다.
  * **50 PPS 소프트랜딩 착지와 잔류 운동 에너지**: 고속($750\,\text{PPS}$)에서 순간 정지하는 것이 아니라, S-Curve 감속 프로파일과 $+4\text{p}$ 마진을 통해 **최저 기동 속도인 $50\,\text{PPS}$ ($5.625^\circ/\text{s}$)까지 속도를 부드럽게 감속한 상태에서 최종 정지**하므로, 정지 시 로터 잔류 운동 에너지($E_k = \frac{1}{2}J\omega^2$)가 고속 순항 대비 **$\mathbf{0.44\%} \left( \frac{50^2}{750^2} = \frac{1}{225} \right)$ 수준으로 급격히 축소**된다.
  * **관성비($J_{\text{ratio}}$) 및 진동 소멸 시간($t_{\text{settle}}$) 수학적 유도 공식**:
    1. **부하 관성 모멘트 적분 수식**:
       $$J_{\text{arm}} = \frac{1}{3} m_{\text{arm}} L_{\text{arm}}^2 = \frac{1}{3} \times 0.03885\,\text{kg} \times (0.080\,\text{m})^2 = 2.88 \times 10^{-5}\,\text{kg}\cdot\text{m}^2$$
       $$J_{\text{lidar}} = m_{\text{lidar}} \cdot r^2 = 0.020\,\text{kg} \times (0.080\,\text{m})^2 = 1.28 \times 10^{-5}\,\text{kg}\cdot\text{m}^2$$
       $$J_{\text{load}} = J_{\text{arm}} + J_{\text{lidar}} = 2.88 \times 10^{-5} + 1.28 \times 10^{-5} = \mathbf{4.16 \times 10^{-5}\,\text{kg}\cdot\text{m}^2}$$
    2. **관성비 도출 ($17\text{HS}4401 \text{ 로터 } J_{\text{rotor}} = 5.4 \times 10^{-6}\,\text{kg}\cdot\text{m}^2$)**:
       $$J_{\text{ratio}} = \frac{J_{\text{load}}}{J_{\text{rotor}}} = \frac{4.16 \times 10^{-5}}{5.4 \times 10^{-6}} = \mathbf{7.70 : 1} \quad (\text{스텝모터 최고 이상치 } 5\sim 10:1 \text{ 완벽 진입})$$
    3. **스텝모터 비틀림 강성($K_t = p \cdot T_h = 20\,\text{N}\cdot\text{m/rad}$) 기반 진동 소멸 시간 수식**:
       $$\omega_n = \sqrt{\frac{K_t}{J_{\text{rotor}} + J_{\text{load}}}} = \sqrt{\frac{20.0}{4.70 \times 10^{-5}}} \approx 652.3\,\text{rad/s}$$
       $$t_{\text{settle}} = \frac{4}{\zeta \omega_n} \approx \frac{4}{0.25 \times 652.3\,\text{rad/s}} \approx \mathbf{25.4\,\text{ms}} \quad (\text{진동 } 98\% \text{ 소멸})$$
  * **40ms 세팅의 하드웨어·통신 정합성 및 정량적 시간 분해 ($25.4\,\text{ms} \to 40.0\,\text{ms}$)**:
    1. **100Hz 라이다 비동기 앨리어싱(Aliasing) 방지 ($+10.0\,\text{ms}$)**:
       * 라이다 발광 측정 클럭과 STM32 모터 타이머는 비동기(Asynchronous)로 동작하므로, 틸트 모터가 $50\,\text{PPS}$에서 정지한 시점과 다음 라이다 거리 측정 패킷 수신 시점 간에는 $0 \sim 10.0\,\text{ms}$의 임의의 위상차가 존재한다.
       * 진동 소멸 직후($25.4\,\text{ms}$) 즉시 Pan 축을 회전시키면, 라이다가 "틸트 정지 + Pan 이동 중"인 불안정 과도 상태에서 샘플링되어 포인트 클라우드 외곽에 **톱니형 왜곡(Jaggies / Ghosting)**이 발생한다.
       * 따라서 라이다가 완전 정지 상태의 깨끗한 끝단 거리를 최소 1회 이상 확정 캡처할 수 있도록 **라이다 1주기($10.0\,\text{ms}$) 대기 마진**을 필연적으로 확보하였다.
    2. **40ms 만료 직후 I2C(100kHz) 3-샘플 중앙값 판독 및 헛 탈조(`ERR_STALL`) 방지 ($+1.2\,\text{ms}$)**:
       * `SC_SWEEP`의 40ms 정착 대기가 만료(`scan_settled() == true`)되는 순간 `SC_LINE_END` 상태로 전이하여 `motor_read_encoder_pulse()`를 통해 MT6701 엔코더를 I2C Standard Mode(100kHz, 1회당 약 $0.38\,\text{ms}$)로 3회 연속 판독(`motor_median3`, 약 $1.2\,\text{ms}$)한다.
       * 기계 진동이 완전히 잦아들기 전에 엔코더를 읽으면, 잔류 진동($\pm 1.0^\circ$)과 엔코더 LSB 지터($0.42^\circ$) 및 1.8° 스텝 데드밴드가 중첩되어 탈조 임계값($2.0^\circ$)을 오인식하는 **헛 탈조(`ERR_STALL`) 스캔 중단 결함**이 발생한다.
       * 진동이 $100\%$ 소멸된 40ms 정착 만료 시점에서 3-샘플 중앙값 필터링(`(a+b+c) - min - max`)을 수행함으로써 I2C 노이즈와 기계적 오판을 100% 차단하고 즉시 다음 Pan 축 스텝(`SC_PAN_STEP`)으로 안전하게 전이한다.
    3. **정량적 40.0ms 시간 분해 불변식**:
       $$T_{\text{settle}} = t_{\text{vibration\_decay}}(25.4\,\text{ms}) + t_{\text{lidar\_period}}(10.0\,\text{ms}) + t_{\text{fsm\_margin}}(4.6\,\text{ms}) = \mathbf{40.0\,\text{ms}} \implies \text{이후 } 1.2\,\text{ms } \text{I2C Median 판정}$$
